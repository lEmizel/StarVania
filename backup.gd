...
enum States { IDLE, RUN, CHUTE, JUMP, WALL_GRIFFE, WALL_JUMP, CLIMB, ROLL, CHUTE_GRIFFE, GRAB, ATTACK_LIGHT_1, ATTACK_LIGHT_2, ATTACK_LIGHT_3, ATTACK_AIR, ATTACK_LOURDE, DEAD, HIT }
...
func apply_damage(amount: int, source_x: float) -> void:
	if current_state == States.ROLL and current_state == States.DEAD:
		return
...
func _raycast_hits_group(rc: RayCast2D, group_name: String, body_only := false) -> bool:
	while rc.is_colliding():
		...
		var is_match := false
		if body_only:
			is_match = col is PhysicsBody2D and col.is_in_group(group_name)
		else:
			is_match = col is Area2D and col.is_in_group(group_name)

		if is_match:
			return true     # trouvé !

		# sinon on ignore ce collider et on continue
...
# Zone "CHUTE" détectée au-dessus + bouton griffe pressé
	...
	# Détection simultanée de deux zones "CLIMB" (droite + gauche)
	var hit_right := _raycast_hits_group(climbcast_right, "CLIMB")   # Area2D par défaut
	...
	if hit_right and hit_left and Input.is_action_just_pressed("griffe"):
		...
		# ─── Recherche de la griffe « forward » ───
	if _raycast_hits_group(climbcast_right, "GRIFFE"):
		...
		return    # on sort en toute fin, qu’on ait changé d’état ou non
...
####################----WALL---JUMP--##########################
###############################################################
## 1) Le RayCast « forward » touche-t-il un mur wall_jump ?
	#var forward_hit = wall_right.is_colliding() \
	#and wall_right.get_collider() is StaticBody2D \
	#and wall_right.get_collider().is_in_group("wall_jump")
#
## 2) Si on NE touche PAS le mur, on autorise le flip basé sur l’input
	#if not forward_hit:
		#_flip_from_input()
#
## 3) Si on touche le mur, on fait un seul flip forcé puis wall-jump
	#if forward_hit:
		#_flip_facing_on_wall()            # retour automatique du sprite
		#change_state(States.WALL_JUMP)
		#return
###############################################################
	...
	# ─── Recherche de la griffe « forward » ───
	while climbcast_right.is_colliding():
		var col := climbcast_right.get_collider()

		# → C'est bien une zone GRIFFE ?
		if col is Area2D and col.is_in_group("GRIFFE"):
			# On exige une vitesse horizontale ≠ 0 et l'input GRIFFE
			if abs(velocity.x) > 0 and Input.is_action_just_pressed("griffe"):
				print("j'ai trouvé la GRIFFE")
				change_state(States.WALL_GRIFFE)
			return       # qu'on entre ou non en WALL_GRIFFE, on sort du bloc

		# → Sinon : on ignore ce collider et on relance le raycast
		climbcast_right.add_exception(col)
		climbcast_right.force_raycast_update()
...
func wall_griffe_execute(_delta: float) -> void:
	# Si le RayCast touche en premier une Area2D du groupe "GRIFFE"…
	if _raycast_hits_group(climbcast_right, "GRIFFE", false):
		return               # …on reste agrippé, rien d’autre à faire

	# Sinon (plus de griffe détectée) → on chute
	change_state(States.CHUTE)
...
func hit_enter() -> void:
	...
	var dir: int = -1 if point.scale.x > 0 else 1   # fallback
	if position_x_enemi != null:
		dir = 1 if (global_position.x - position_x_enemi) > 0 else -1
	velocity.x = dir * 150      # monstre : 250  – joueur : 150 si tu veux
	velocity.y = -150.0
